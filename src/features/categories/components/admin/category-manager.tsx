...
export function CategoryManager() {
  ...
  const { data: categories = [], isLoading } = useQuery(
    CategoriesListQueryOptions({ withCount: true }),
  );
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {isLoading ? (
        ...
        <div className="border border-border/30 divide-y divide-border/30">
          ...
          <div className="hidden md:grid grid-cols-12 gap-4 px-4 py-3 bg-muted/5 text-[9px] font-mono uppercase tracking-[0.2em] text-muted-foreground">
            ...
            <div className="col-span-1">文章数</div>
          ...
          {filtered.map((c) => (
            ...
            >
              ...
              <div className="col-span-1 text-xs font-mono text-indigo-500">
                {c.postCount ?? 0}
              </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full min-h-20 rounded-none border border-border/40 bg-transparent px-3 py-2 text-sm text-foreground focus:outline-none focus:border-foreground transition-colors"
                  placeholder="分类描述"
                />
              </div>
              <div>
      ...
      <ConfirmationModal
        ...
        title="删除分类"
        message={`确定要删除分类「${deleting?.name ?? ""}」吗？此操作不可撤销。`}
        confirmLabel="删除"
