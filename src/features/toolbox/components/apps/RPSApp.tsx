        ...
        <div className="flex flex-col items-center gap-1">
          <span
            className={`text-lg font-black transition-colors ${
              result
                ? OUTCOME_CONFIG[result.outcome].color
          ...
          <div className="flex h-2.5 rounded-full overflow-hidden bg-slate-200 dark:bg-slate-700">
            {stats.win > 0 && (
              <div
                ...
                style={{ width: `${(stats.win / stats.total) * 100}%` }}
              />
            )}
            {stats.draw > 0 && (
              <div
                className="bg-amber-400 transition-all duration-500"
                style={{ width: `${(stats.draw / stats.total) * 100}%` }}
              />
            )}
            {stats.lose > 0 && (
              <div
                className="bg-rose-400 transition-all duration-500"
                style={{ width: `${(stats.lose / stats.total) * 100}%` }}
              />
            )}
          </div>
