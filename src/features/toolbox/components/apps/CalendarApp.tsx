...
// 0=小月29天, 1=大月30天
const lunarYearData: Record<
  number,
  { leap: number; months: number[]; leapDays?: number }
> = {
...
export default function CalendarApp() {
  ...
  const getDayInfo = (d: number) => {
    ...
    const holiday =
      solarHolidays[solarKey] ||
      lunarHolidays[lunarKey] ||
      solarTerms[termKey] ||
      "";
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <button
          type="button"
          onClick={goToday}
          className="text-sm font-bold text-slate-800 dark:text-slate-200 hover:text-indigo-500 transition-colors"
        >
          {year}年{month + 1}月
        </button>
        <button
      ...
      <div className="grid grid-cols-7 gap-0.5 text-center mb-3">
        {WEEKDAYS.map((d, i) => (
          <div
            key={d}
            className={`text-[10px] font-bold py-1 ${i === 0 || i === 6 ? "text-rose-400" : "text-slate-400 dark:text-slate-500"}`}
          >
            {d}
          </div>
        ))}
        {days.map((d, i) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`text-[7px] leading-none mt-0.5 truncate w-full px-0.5 ${
                  holiday
              ...
              {memoFlag && (
                <div className="absolute top-0.5 right-1 w-1 h-1 rounded-full bg-pink-500" />
              )}
            </button>
          );
        })}
          ...
          <div className="flex items-center justify-between mb-1">
            <div>
              ...
              <span className="text-xs text-slate-400 dark:text-slate-500 ml-2">
                {selectedInfo.lunar.monthName}
                {selectedInfo.lunar.dayName}
              </span>
              {selectedInfo.holiday && (
                <span className="text-xs text-amber-500 ml-2 font-bold">
                  {selectedInfo.holiday}
                </span>
              )}
            </div>
            ...
            {editingMemo ? (
              <div className="flex gap-1">
                ...
                <button
                  type="button"
                  onClick={saveMemo}
                  className="px-2 py-1 rounded-lg bg-indigo-500 text-white text-xs font-bold"
                >
                  保存
                </button>
              </div>
