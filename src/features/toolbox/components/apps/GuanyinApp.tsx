...
export default function GuanyinApp() {
  ...
  const colors = data?.fortune
    ? FORTUNE_COLORS[data.fortune] || FORTUNE_COLORS["中签"]
    : null;
      ...
      {data && colors && (
        ...
        >
          ...
          <div className="text-center space-y-1">
            <div className={`text-base font-black ${colors.text}`}>
              {data.name}
            </div>
            <div className="flex items-center justify-center gap-2">
              <span
                className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${colors.text} ${colors.bg} border ${colors.border}`}
              >
                {data.fortune}
              </span>
          ...
          {data.poem_version_1 && (
            <div className="text-center">
              <div className="text-xs text-slate-700 dark:text-slate-300 leading-loose whitespace-pre-line">
                {data.poem_version_1}
              </div>
            </div>
          )}
          ...
          {data.poem_version_2 && (
            <div>
              ...
              <div className="text-[11px] text-slate-600 dark:text-slate-300 leading-loose whitespace-pre-line">
                {data.poem_version_2}
              </div>
            </div>
          )}
