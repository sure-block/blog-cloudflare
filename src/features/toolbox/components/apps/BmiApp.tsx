...
function getBmiLevel(bmi: number): {
  ...
  if (bmi < 18.5)
    return { label: "偏瘦", color: "text-blue-500", bg: "bg-blue-500" };
  if (bmi < 24)
    return { label: "正常", color: "text-green-500", bg: "bg-green-500" };
  if (bmi < 28)
    return { label: "偏胖", color: "text-amber-500", bg: "bg-amber-500" };
  ...
  const handleCalc = async () => {
    ...
    try {
      const res = await fetch(
        `https://v2.xxapi.cn/api/bmi?height=${h}&weight=${w}`,
      );
