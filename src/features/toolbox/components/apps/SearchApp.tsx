              ...
              {(["relevance", "date"] as SortType[]).map((s) => (
                <button
                  ...
                  className={`px-2 py-0.5 rounded-full text-[10px] font-medium transition-colors ${
                    sort === s
                      ? "bg-sky-500 text-white"
                      : "bg-white dark:bg-slate-900 text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700"
                  }`}
                >
                  {s === "relevance" ? "相关性" : "时间"}
                </button>
          ...
          {result.results.map((item, i) => (
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                ...
                />
                <span className="text-[9px] text-green-600 dark:text-green-400 truncate">
                  {item.domain}
                </span>
                {item.publish_time && (
                  <span className="text-[9px] text-slate-400 tabular-nums ml-auto shrink-0">
                    {fmtTime(item.publish_time)}
                  </span>
                )}
              </div>
              <h4 className="text-xs font-bold text-sky-700 dark:text-sky-400 line-clamp-1 mb-0.5">
                {item.title}
              </h4>
