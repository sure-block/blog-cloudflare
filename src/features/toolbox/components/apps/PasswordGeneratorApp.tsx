...
export default function PasswordGeneratorApp() {
  ...
  const generate = useCallback(() => {
    ...
    if (batchCount > 1) {
      ...
      setHistory((h) =>
        [
          { password: pw, strength: strength.score, time: Date.now() },
          ...h,
        ].slice(0, 10),
      );
    }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        >
          <span
            className={`w-3 h-3 rounded-sm border flex items-center justify-center ${options.excludeAmbiguous ? "bg-amber-500 border-amber-500" : "border-slate-300 dark:border-slate-600"}`}
          >
            {options.excludeAmbiguous && (
