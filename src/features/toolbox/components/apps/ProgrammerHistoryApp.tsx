...
export default function ProgrammerHistoryApp() {
  ...
  useEffect(() => {
    ...
    fetch("/api/uapis?path=history/programmer/today")
      .then((res) => res.json().then((d: any) => ({ ok: res.ok, data: d })))
      .then(({ ok, data }) => {
          ...
          {data.events.map((event, i) => (
            ...
            >
              <div className="flex items-center gap-2 mb-1.5">
                ...
                {event.category && (
                  <span
                    className={`text-[9px] px-1.5 py-0.5 rounded-full font-medium ${catColors[event.category] || "bg-slate-100 dark:bg-slate-700 text-slate-500"}`}
                  >
                    {event.category}
                  </span>
                )}
