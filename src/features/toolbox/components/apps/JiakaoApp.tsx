...
export default function JiakaoApp() {
  ...
  const getOptionStyle = (opt: string, _label: string) => {
    if (!showResult) {
      return selected === opt
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-[10px] text-slate-400">
          答对 <span className="font-bold text-green-500">{correct}</span> /{" "}
          {total}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex items-center gap-2">
            <span className="text-[9px] px-1.5 py-0.5 rounded bg-indigo-50 dark:bg-indigo-900/20 text-indigo-500 font-medium">
              {q.chapter}
            </span>
