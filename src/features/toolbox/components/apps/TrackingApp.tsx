        ...
        <div className="flex-1 overflow-y-auto space-y-3">
          ...
          <div className="rounded-xl bg-gradient-to-br from-sky-500/10 to-indigo-500/10 dark:from-sky-500/20 dark:to-indigo-500/20 border border-sky-500/20 p-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <span
                className={`text-[10px] px-2 py-0.5 rounded-full text-white font-medium ${statusColors[result.status_code] || "bg-slate-400"}`}
              >
                {result.status}
              </span>
            </div>
            ...
            {result.tracks.map((track, i) => (
              <div key={i} className="flex gap-3">
                ...
                <div className="flex flex-col items-center shrink-0 w-5">
                  <div
                    className={`w-2.5 h-2.5 rounded-full shrink-0 ${i === 0 ? "bg-sky-500" : "bg-slate-300 dark:bg-slate-600"}`}
                  />
                ...
                <div className="pb-3 min-w-0">
                  <p
                    className={`text-xs leading-relaxed ${i === 0 ? "text-slate-800 dark:text-slate-200 font-medium" : "text-slate-500 dark:text-slate-400"}`}
                  >
                    {track.context}
                  </p>
