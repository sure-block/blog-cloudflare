...
function resolveCollision(a: Fruit, b: Fruit) {
  ...
  const impulse =
    ((1 + RESTITUTION) * dvDotN) / (1 / (a.r * a.r) + 1 / (b.r * b.r));
...
export default function WatermelonGame() {
  ...
  const tick = useCallback((dt: number) => {
    ...
    fruits.forEach((f) => {
      ...
      // Settled check
      f.settled =
        Math.abs(f.vx) < 2 && Math.abs(f.vy) < 2 && f.y + f.r >= H - 6;
    });
    ...
    const dangerFruits = fruitsRef.current.filter(
      (f) => f.settled && f.y - f.r < WALL_Y,
    );
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (!rect) return;
    dropX.current = Math.max(
      20,
      Math.min(W - 20, (e.clientX - rect.left) * (W / rect.width)),
    );
  };
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    dropX.current = Math.max(
      20,
      Math.min(W - 20, (e.touches[0].clientX - rect.left) * (W / rect.width)),
    );
  };
