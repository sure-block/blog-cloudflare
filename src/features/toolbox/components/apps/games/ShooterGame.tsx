...
export default function ShooterGame() {
  ...
  const draw = useCallback(() => {
    ...
    bulletsRef.current.forEach((b) => {
      if (!isFinite(b.x) || !isFinite(b.y) || !isFinite(b.h) || b.h <= 0)
        return;
  ...
  const tick = useCallback(
    (dt: number) => {
      ...
      if (multiShotRef.current > 0)
        multiShotRef.current = Math.max(0, multiShotRef.current - dt);
      ...
      if (screenShake.current > 0)
        screenShake.current = Math.max(0, screenShake.current - dt * 4);
          ...
          if (Math.random() < dt * 2) {
            ...
            if (
              ...
              const angle = Math.atan2(
                player.y + PLAYER_H / 2 - by,
                player.x + PLAYER_W / 2 - bx,
              );
              bullets.push({
                x: bx,
                y: by,
          ...
          if (overlap(bulletBox, e)) {
            ...
            if (e.hp <= 0) {
              ...
              if (e.type === "boss") {
                ...
                waveEnemiesLeft.current = Math.max(
                  0,
                  waveEnemiesLeft.current - 1,
                );
              }
              ...
              const comboMult =
                comboRef.current >= 5 ? 3 : comboRef.current >= 3 ? 2 : 1;
      ...
      for (let i = powerUps.length - 1; i >= 0; i--) {
        ...
        if (overlap(puBox, playerBox)) {
          ...
          if (pu.type === "shield") shieldRef.current = 5;
            ...
            bombRef.current = Math.min(bombRef.current + 1, 3);
        }
      }
  ...
  useEffect(() => {
    const down = (e: KeyboardEvent) => {
      if (e.key === "ArrowLeft" || e.key === "a") {
        e.preventDefault();
        moveDir.current.left = true;
      }
      if (e.key === "ArrowRight" || e.key === "d") {
        e.preventDefault();
        moveDir.current.right = true;
      }
      if (e.key === "ArrowUp" || e.key === "w") {
        e.preventDefault();
        moveDir.current.up = true;
      }
      if (e.key === "ArrowDown" || e.key === "s") {
        e.preventDefault();
        moveDir.current.down = true;
      }
      if (e.key === " " || e.key === "b") {
        e.preventDefault();
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    playerRef.current.x = clamp(
      touchRef.current.px + (tx - touchRef.current.x),
      0,
      W - PLAYER_W,
    );
    playerRef.current.y = clamp(
      touchRef.current.py + (ty - touchRef.current.y),
      0,
      H - PLAYER_H,
    );
  };
