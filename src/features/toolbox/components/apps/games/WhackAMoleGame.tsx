...
export default function WhackAMoleGame() {
  const [moles, setMoles] = useState<Mole[]>(() =>
    Array(9)
      .fill(null)
      .map(() => ({ state: "idle", timer: 0, type: "normal" })),
  );
  ...
  const initGame = useCallback(() => {
    const empty = Array(9)
      .fill(null)
      .map(() => ({ state: "idle", timer: 0, type: "normal" }) as Mole);
    ...
    <div className="flex flex-col items-center gap-2 md:gap-3 w-full select-none py-2">
      ...
      <div className="flex items-center justify-center gap-3 w-full">
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-1.5 text-center min-w-[60px]">
          ...
          <div
            className={`text-sm font-black ${timeLeft <= 5 ? "text-red-500" : "text-blue-700 dark:text-blue-400"}`}
          >
            {timeLeft}s
          </div>
        </div>
        ...
        >
          {moles.map((mole, i) => (
            ...
            >
              ...
              {/* Mole */}
              <span
                className={`text-3xl md:text-4xl transition-transform duration-150 ${
                  mole.state === "up"
                    ? "translate-y-0"
                    : mole.state === "hit"
