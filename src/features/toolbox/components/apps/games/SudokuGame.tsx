...
function generatePuzzle(clues: number): {
  puzzle: number[][];
  solution: number[][];
} {
...
export default function SudokuGame() {
  ...
  const handleNumber = (num: number) => {
    ...
    if (notesMode) {
      ...
      // Check conflicts
      for (let i = 0; i < 9; i++) {
        if (i !== c && newGrid[r][i].value === num) {
          newGrid[r][i].error = true;
          newGrid[r][c].error = true;
        }
        if (i !== r && newGrid[i][c].value === num) {
          newGrid[i][c].error = true;
          newGrid[r][c].error = true;
        }
      }
      const br = Math.floor(r / 3) * 3;
      ...
      for (let dr = 0; dr < 3; dr++)
        for (let dc = 0; dc < 3; dc++) {
          ...
          if (newGrid[nr][nc].value === num) {
            newGrid[nr][nc].error = true;
            newGrid[r][c].error = true;
          }
        }
        ...
        <div className="grid grid-cols-9 gap-px">
          {grid.map((row, r) =>
            row.map((cell, c) => {
              ...
              const isSameNum =
                highlightNum !== 0 && cell.value === highlightNum;
                  ...
                  {cell.value !== 0 ? (
                    ...
                    <div className="grid grid-cols-3 gap-0 leading-none">
                      {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((n) => (
                        <span
                          key={n}
                          className={`text-[6px] md:text-[7px] text-slate-400 ${cell.notes.has(n) ? "visible" : "invisible"}`}
                        >
                          {n}
                        </span>
                      ))}
