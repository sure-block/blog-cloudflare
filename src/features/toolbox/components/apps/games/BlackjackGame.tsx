...
function _isSoft(hand: Card[]): boolean {
  let total = 0;
  let aces = 0;
  for (const c of hand) {
    if (c.hidden) continue;
...
export default function BlackjackGame() {
  const [_deck, _setDeck] = useState<Card[]>(createDeck);
  ...
  const [best, setBest] = useState(() => {
    try {
      return Number(localStorage.getItem(STORAGE_KEY)) || 1000;
    } catch {
      return 1000;
    }
  });
  ...
  const drawCard = useCallback(
    (d: Card[], hidden = false): { card: Card; remaining: Card[] } => {
  ...
  const renderCard = (card: Card, i: number) => {
    if (card.hidden) {
      return (
        <div
          key={i}
          className="w-12 h-16 md:w-14 md:h-[72px] rounded-lg bg-gradient-to-br from-blue-600 to-blue-800 border-2 border-blue-400 flex items-center justify-center"
        >
          <svg
    ...
    return (
      <div
        key={i}
        className="w-12 h-16 md:w-14 md:h-[72px] rounded-lg bg-white dark:bg-slate-100 border border-slate-200 shadow-sm flex flex-col items-center justify-center gap-0 relative"
      >
        <span
          className={`text-[10px] font-bold absolute top-1 left-1.5 ${color}`}
        >
          {card.rank}
        </span>
        <span className={`text-lg md:text-xl ${color}`}>{card.suit}</span>
        <span
          className={`text-[10px] font-bold absolute bottom-1 right-1.5 rotate-180 ${color}`}
        >
          {card.rank}
        </span>
      </div>
  ...
  return (
    <div className="flex flex-col items-center gap-2 md:gap-3 w-full max-w-[280px] md:max-w-xs mx-auto select-none py-2">
      ...
      {message && (
        <div
          className={`text-center px-3 py-1 rounded-lg text-xs font-bold ${
            message.includes("赢") ||
        ...
        <div className="flex flex-col items-center gap-2 w-full">
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={bet <= 10}
              className="w-8 h-8 rounded-lg bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold text-sm hover:bg-slate-300 dark:hover:bg-slate-600 disabled:opacity-30 active:scale-95 transition-all"
            >
              -
            </button>
            <span className="text-sm font-bold text-slate-700 dark:text-slate-300 w-16 text-center">
              {bet}
            </span>
            <button
              type="button"
      ...
      {state === "dealer" && (
        <div className="text-center text-sm text-slate-500 animate-pulse font-bold">
          庄家思考中...
        </div>
      )}
