...
export default function SnakeGame() {
  ...
  const spawnParticles = useCallback(
    (x: number, y: number, color: string, count = 6) => {
    ...
    foodsRef.current.forEach((f) => {
      ...
      if (f.ttl !== undefined) {
        ...
        ctx.arc(
          ...
          r + 2,
          -Math.PI / 2,
          -Math.PI / 2 + Math.PI * 2 * (f.ttl / (f.type === "golden" ? 8 : 10)),
        );
        ctx.stroke();
      }
    });
  ...
  const tick = useCallback(() => {
    ...
    if (
      !isGhost &&
      wallsRef.current.some((w) => w.pos.x === head.x && w.pos.y === head.y)
    ) {
      setGameOver(true);
      setRunning(false);
      return;
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (ghostRef.current > 0)
        ghostRef.current = Math.max(0, ghostRef.current - dt);
      ...
      if (foodsRef.current.length === 0) {
        foodsRef.current.push(
          randomFood(snakeRef.current, wallsRef.current, foodsRef.current),
        );
      }
          ...
          <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
            <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 text-center space-y-3">
              ...
              <div className="text-sm text-slate-500">
                得分：{score} · 长度：{snakeRef.current.length}
              </div>
              <button
                type="button"
                onClick={restart}
      ...
      <div className="flex items-center justify-center gap-3 w-full text-[10px]">
        {ghostRef.current > 0 && (
          <span className="text-purple-500 font-bold">
            👻 穿墙 {ghostRef.current.toFixed(0)}s
          </span>
        )}
        {slowRef.current > 0 && (
          <span className="text-blue-500 font-bold">
            ❄ 减速 {slowRef.current.toFixed(0)}s
          </span>
        )}
      </div>
