...
export default function DoodleJumpGame() {
  ...
  const draw = useCallback(() => {
    ...
    for (let gx = 0; gx < W; gx += 24)
      for (let gy = 0; gy < H; gy += 24) ctx.fillRect(gx, gy, 1, 1);
    ...
    for (const p of s.plats) {
      ...
      if (p.type === "breakable") {
        ...
        ctx.moveTo(p.x + 38, p.y + 3);
        ctx.lineTo(p.x + 42, p.y + 7);
        ctx.stroke();
      }
    }
  ...
  const onTouchMove = (e: React.TouchEvent) => {
    ...
    state.current.px = Math.max(
      ...
      Math.min(
        W - PW,
        ((e.touches[0].clientX - rect.left) / rect.width) * W - PW / 2,
      ),
    );
  };
  const onTouchEnd = () => {
    state.current.pdir = 0;
  };
