...
const ITEM_DEFS: Record<
  ItemType,
  { emoji: string; points: number; r: number; color: string; chance: number }
> = {
  coin_s: { emoji: "🪙", points: 10, r: 12, color: "#f59e0b", chance: 0.35 },
  coin_m: { emoji: "💰", points: 25, r: 14, color: "#f59e0b", chance: 0.2 },
  coin_l: { emoji: "💎", points: 50, r: 16, color: "#3b82f6", chance: 0.1 },
...
export default function CoinCatcherGame() {
  ...
  const draw = useCallback(() => {
    ...
    ctx.fill();
    // Basket body
    const baskGrad = ctx.createLinearGradient(
      bx,
      BASKET_Y,
      bx,
      BASKET_Y + BASKET_H,
  ...
  const tick = useCallback((dt: number) => {
    ...
    if (moveDir.current.right)
      basketX.current = Math.min(W - BASKET_W, basket + 300 * dt);

    // Power-up timers
    if (starRef.current > 0)
      starRef.current = Math.max(0, starRef.current - dt);
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    basketX.current = Math.max(
      0,
      Math.min(W - BASKET_W, tx - touchRef.current.offsetX),
    );
  };
