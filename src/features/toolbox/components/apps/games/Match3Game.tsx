...
function createGrid(): Gem[][] {
  const grid: Gem[][] = Array.from({ length: ROWS }, () =>
  ...
  for (let r = 0; r < ROWS; r++)
    for (let c = 0; c < COLS; c++) {
      ...
      while (
        (c >= 2 &&
          grid[r][c - 1].type === gem.type &&
          grid[r][c - 2].type === gem.type) ||
        (r >= 2 &&
          grid[r - 1][c].type === gem.type &&
          grid[r - 2][c].type === gem.type)
      ) {
        gem = createGem();
      }
      grid[r][c] = gem;
...
export default function Match3Game() {
  ...
  const _matchedRef = useRef<Set<string>>(new Set());
    ...
    <div className="flex flex-col items-center gap-2 md:gap-3 w-full select-none py-2">
      ...
      <div className="flex items-center justify-center gap-3 w-full">
        ...
        <div className="bg-slate-100 dark:bg-slate-800 rounded-lg p-1.5 text-center min-w-[60px]">
          ...
          <div
            className={`text-sm font-black ${moves <= 5 ? "text-red-500" : "text-slate-800 dark:text-white"}`}
          >
            {moves}
          </div>
        </div>
          ...
          {grid.map((row, r) =>
            row.map((gem, c) => {
              ...
              return (
                ...
                >
                  {gem ? (
                    <span className={isMatched ? "animate-pulse" : ""}>
                      {GEM_EMOJIS[gem.type]}
                    </span>
                  ) : null}
                </button>
