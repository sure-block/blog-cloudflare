...
export default function MemoryGame() {
  ...
  const [bests, setBests] = useState<
    Record<string, { moves: number; time: number }>
  >(() => {
    try {
  ...
  const saveBest = (m: number, t: number) => {
    setBests((prev) => {
      ...
      if (
        prevBest &&
        (m > prevBest.moves || (m === prevBest.moves && t >= prevBest.time))
      )
        return prev;
        ...
        {cards.map((card) => (
          ...
          >
            ...
            >
              ...
              {/* 正面 */}
              <div
                className={`absolute inset-0 [backface-visibility:hidden] [transform:rotateY(180deg)] rounded-lg flex items-center justify-center ${
                  card.matched
                    ? "bg-green-100 dark:bg-green-900/30"
                    : "bg-white dark:bg-slate-700"
                }`}
              >
                <span className={card.matched ? "opacity-60" : ""}>
                  {card.emoji}
                </span>
              </div>
            </div>
