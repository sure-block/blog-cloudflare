...
export default function CoinFlipApp() {
  ...
  const flip = () => {
    ...
    setTimeout(() => {
      ...
      setResult(side);
      setHistory((prev) =>
        [{ side, id: nextId.current++ }, ...prev].slice(0, 50),
      );
      setFlipping(false);
    }, 600);
  };
      ...
      <div className="flex flex-col items-center justify-center flex-1 gap-4">
        ...
        >
          ...
          >
            {flipping ? (
              ...
              <svg
                className="w-14 h-14 text-amber-700"
                viewBox="0 0 24 24"
                fill="currentColor"
              >
                <circle
              ...
              >
                <circle cx="12" cy="12" r="10" />
                <path
                  d="M9 9h.01M15 9h.01M9.5 15c.83.67 1.67 1 2.5 1s1.67-.33 2.5-1"
                  strokeLinecap="round"
                />
              </svg>
            )}
        ...
        {result && !flipping && (
          <div className="text-center">
            <span
              className={`text-lg font-black ${result === "heads" ? "text-amber-600 dark:text-amber-400" : "text-slate-600 dark:text-slate-300"}`}
            >
              {result === "heads" ? "正面" : "反面"}
            </span>
          </div>
            ...
            <div className="flex items-center gap-3">
              <span className="flex items-center gap-1">
                ...
                <span className="text-slate-600 dark:text-slate-300">
                  正面 {stats.heads} ({headsPercent}%)
                </span>
              </span>
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 rounded-full bg-slate-400 dark:bg-slate-500" />
                <span className="text-slate-600 dark:text-slate-300">
                  反面 {stats.tails} ({tailsPercent}%)
                </span>
              </span>
            </div>
