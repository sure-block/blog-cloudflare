...
function DiceFace({ value, rolling }: { value: number; rolling: boolean }) {
  ...
  return (
    <svg
      viewBox="0 0 100 100"
      className={`w-full h-full ${rolling ? "animate-spin" : ""}`}
    >
      <rect
        x="5"
        y="5"
...
function DiceNumber({
  ...
  return (
    <div
      className={`w-full h-full flex items-center justify-center rounded-xl border-2 transition-all ${
        isMax
    ...
    >
      <span
        className={`text-2xl font-black ${isMax ? "text-amber-500" : "text-indigo-600 dark:text-indigo-400"}`}
      >
        {value}
      </span>
    </div>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-3">
        {currentResult ? (
          <>
            ...
            >
              {currentResult.values.map((v, i) => (
                <div
                  key={i}
                  className="aspect-square w-full max-w-[72px] mx-auto"
                >
                  {diceType === 6 ? (
          ...
          <div className="text-slate-300 dark:text-slate-600 text-sm">
            点击下方按钮投掷
          </div>
        )}
      </div>
