      ...
      {result && !loading && (
        <div className="flex-1 overflow-y-auto space-y-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-1.5">
                  <a
                    ...
                    className="text-sm font-bold text-slate-800 dark:text-slate-200 hover:text-sky-500 transition-colors truncate"
                  >
                ...
                {result.organizations.map((org) => (
                  ...
                  >
                    <img
                      src={org.avatar_url}
                      alt={org.login}
                      className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-700 hover:ring-2 hover:ring-sky-500/40 transition-all"
                    />
                  </a>
                ))}
              </div>
            ...
            <div className="rounded-xl bg-white/50 dark:bg-slate-800/50 border border-slate-200/50 dark:border-white/5 p-3">
              <div className="flex items-center justify-between mb-2">
                ...
                <p className="text-[9px] text-slate-400">
                  近一年{" "}
                  {result.activity.contribution_calendar.total_contributions}{" "}
                  次贡献
                </p>
              </div>
              <div className="overflow-x-auto">
                <div className="inline-flex gap-px">
                  ...
                  {/* 格子 */}
                  {result.activity.contribution_calendar.weeks.map(
                    (week, wi) => (
                      <div key={wi} className="flex flex-col gap-px">
                        {Array.from({ length: 7 }, (_, di) => {
                          const day = week.contribution_days.find(
                            (d) => d.weekday === di,
                          );
                          return (
                            <div
                              ...
                              title={
                                day
                                  ? `${day.date}: ${day.contribution_count} 次贡献`
                                  : ""
                              }
                            />
                          );
              ...
              <div className="flex flex-wrap gap-x-3 gap-y-1 mt-2">
                <span className="text-[9px] text-slate-400">
                  Commit {result.activity.total_commit_contributions}
                </span>
                <span className="text-[9px] text-slate-400">
                  Issue {result.activity.total_issue_contributions}
                </span>
                <span className="text-[9px] text-slate-400">
                  PR {result.activity.total_pull_request_contributions}
                </span>
                <span className="text-[9px] text-slate-400">
                  Review{" "}
                  {result.activity.total_pull_request_review_contributions}
                </span>
              </div>
              {/* 月度时间线 */}
                    ...
                    >
                      <div className="w-full h-8 flex items-end justify-center">
                        <div
                          ...
                          style={{
                            height: `${Math.max(4, Math.min(32, t.contribution_count * 8))}px`,
                          }}
                        />
                      </div>
          ...
          {result.pinned_repositories &&
            result.pinned_repositories.length > 0 && (
              <div>
                  ...
                  {result.pinned_repositories.map((repo) => (
                    ...
                    >
                      <div className="flex items-center gap-1 mb-0.5">
                        ...
                        {repo.archived && (
                          <span className="text-[8px] px-1 py-0.5 rounded-full bg-amber-500/10 text-amber-600 dark:text-amber-400">
                            归档
                          </span>
                        )}
                      </div>
