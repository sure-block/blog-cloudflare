  ...
  async function handleSearch() {
    ...
    try {
      const res = await fetch(
        `/api/uapis?path=github/repo&repo=${encodeURIComponent(repo)}`,
      );
        ...
        <div className="flex-1 overflow-y-auto space-y-3">
          ...
          <div className="grid grid-cols-4 gap-2">
            {[
              ...
              <div
                key={s.label}
                className="text-center rounded-xl bg-white/50 dark:bg-slate-800/50 border border-slate-200/50 dark:border-white/5 py-2"
              >
              ...
              <div className="flex flex-wrap gap-2">
                {langEntries.map(([lang, bytes], i) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className={`w-2 h-2 rounded-full ${colors[i].replace("text-", "bg-")}`}
                      />
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs font-bold text-slate-800 dark:text-slate-200">
                  {result.latest_release.tag_name}
                </span>
              ...
              <p className="text-[9px] text-slate-400 mt-0.5">
                {fmtDate(result.latest_release.published_at)}
              </p>
            </a>
          )}
          ...
          {result.topics.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {result.topics.map((t) => (
                <span
                  key={t}
                  className="text-[9px] px-2 py-0.5 rounded-full bg-sky-500/10 text-sky-600 dark:text-sky-400"
                >
                  {t}
                </span>
              ))}
      ...
      {!result && !loading && !error && (
        <div className="flex-1 flex items-center justify-center">
          <p className="text-[10px] text-slate-400 text-center">
            输入 GitHub 仓库名查询，格式如 owner/repo
          </p>
        </div>
      )}
    </div>
