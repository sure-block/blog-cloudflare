...
const _DIM_BG: Record<string, string> = {
  all: "bg-indigo-500",
  love: "bg-pink-500",
  work: "bg-sky-500",
  money: "bg-amber-500",
    ...
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => (
        <svg
          ...
          className="w-3 h-3"
          viewBox="0 0 20 20"
          fill={i <= count ? "#fbbf24" : "currentColor"}
        >
...
export default function HoroscopeApp() {
  ...
  const _currentSign = SIGNS.find((s) => s.id === sign);
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex gap-1 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-none">
        {SIGNS.map((s) => (
          ...
          >
            <span
              className={`text-lg ${sign === s.id ? "text-indigo-500" : "text-slate-400"}`}
            >
              {s.symbol}
            </span>
            <span
              className={`text-[9px] font-bold ${sign === s.id ? "text-indigo-600 dark:text-indigo-400" : "text-slate-500"}`}
            >
              {s.label}
            </span>
          </button>
      ...
      {loading ? (
        ...
        <>
            ...
            {["all", "love", "work", "money", "health"].map((dim) => {
              ...
              return (
                <div key={dim}>
                  ...
                  >
                    <span
                      className={`text-[10px] font-bold w-7 ${DIM_COLORS[dim]}`}
                    >
                      {DIM_LABELS[dim]}
                    </span>
          ...
          {data.todo && (
            <div className="flex gap-2">
              <div className="flex-1 bg-green-50 dark:bg-green-900/20 rounded-lg p-2">
                <div className="text-[9px] font-bold text-green-500 mb-0.5">
                  宜
                </div>
          ...
          <div className="text-[9px] text-slate-300 dark:text-slate-600 text-right">
            {data.time}
          </div>
        </>
      ) : (
